...
#### オブジェクト指向による部品化のルール
現実世界に出てくる登場人物の単位で、プログラムをクラスに分解する。
#### オブジェクト指向の本質
...
- カプセル化
#### インスタンスとクラスの関係
仮想世界で活動するのはインスタンスであり、そのインスタンスを生み出すための金型がクラスである。
#### フィールドの宣言
...
#### クラス型変数を用いる理由
仮想世界に複数存在しうる同名インスタンスの中から、特定の1つのインスタンスをプログラムとして識別するため。
#### インスタンスの生成
~~~java
クラス名 変数名=new クラス名();
~~~
...
オーバーライド時にメソッド名を間違えていたりするとコンパイラが教えてくれる。また、**明示的にオーバーライドしている**メソッドだと他のプログラマに知らせる事ができる。
#### 継承やオーバーライドの禁止
